  ...
  <div class="space-y-8">
    ...
    <div class="bg-white p-5 rounded-xl border border-sage-100 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
      ...
      <div class="flex items-center gap-3">
        ...
        <div class="flex items-center bg-sage-50 p-1 rounded-lg text-xs font-semibold text-sage-700 border border-sage-200/60">
          <button
            @click="viewMode = 'table'"
            class="px-3 py-1.5 rounded-md transition-all"
            :class="viewMode === 'table' ? 'bg-white text-forest-900 shadow-xs font-bold' : 'hover:text-forest-900'"
          >
            Tabel
          </button>
          <button
            @click="viewMode = 'card'"
            class="px-3 py-1.5 rounded-md transition-all"
            :class="viewMode === 'card' ? 'bg-white text-forest-900 shadow-xs font-bold' : 'hover:text-forest-900'"
          >
            Card
          </button>
        </div>
        ...
        <button
          ...
          class="px-4 py-2.5 rounded-lg bg-peach-500 hover:bg-peach-600 text-white text-xs font-bold shadow-xs transition-all flex items-center gap-1.5"
        >
        ...
        <table class="w-full text-left border-collapse text-xs min-w-[760px]">
          ...
          <tbody class="divide-y divide-stone-100 text-stone-800">
            ...
            >
              ...
              <td class="py-3.5 px-4 text-left">
                <span v-if="item.formulaBaseName" class="px-2 py-0.5 rounded bg-stone-100 text-stone-700 border border-stone-200 text-[11px] font-medium whitespace-nowrap">
              ...
              <!-- Grand Total HPP -->
              <td class="py-3.5 px-4 text-left">
                <span class="font-mono font-bold text-amber-950 text-xs px-2.5 py-1 rounded-md bg-amber-50 border border-amber-200 inline-block">
    ...
    <div v-else class="space-y-4">
      <div v-if="filteredHppList.length === 0" class="py-12 text-center text-stone-400 bg-white rounded-xl border">
        <Receipt class="w-8 h-8 mx-auto mb-2 opacity-50" />
        <p class="text-xs text-stone-400">Tidak ada data HPP ditemukan.</p>
      </div>
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        <div
          v-for="item in paginatedHppList"
          :key="item.id"
          class="bg-white rounded-xl border border-stone-200/80 p-5 shadow-sm space-y-4 hover:shadow-md transition-all flex flex-col justify-between"
        >
        <div>
          ...
          <div class="mt-3 space-y-2 text-xs">
            ...
            <div class="flex justify-between text-emerald-800 bg-emerald-50 p-2 rounded-lg border border-emerald-200">
        ...
        <div class="pt-3 border-t border-stone-100 flex items-center justify-between">
          ...
          <div class="flex items-center gap-1.5">
            <button
              @click="viewDetail(item)"
              class="p-1.5 rounded-md border border-blue-200 bg-blue-50/70 hover:bg-blue-100 text-blue-600 hover:text-blue-700 text-xs transition-colors"
              title="Lihat Detail HPP"
            >
              <Eye class="w-3.5 h-3.5" />
            </button>
            <button
              @click="confirmDelete(item)"
              class="p-1.5 rounded-md border border-rose-200 bg-rose-50/70 hover:bg-rose-100 text-rose-600 hover:text-rose-700 text-xs transition-colors"
              title="Hapus Data HPP"
            >
              <Trash2 class="w-3.5 h-3.5" />
            </button>
    ...
    <div class="bg-stone-900 text-stone-100 rounded-2xl p-6 sm:p-8 border border-stone-800 shadow-xl space-y-6">
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-stone-800 pb-5">
        ...
        <div class="flex items-center gap-2">
          ...
          <button
            ...
            class="px-3 py-1.5 rounded-lg bg-stone-800 hover:bg-stone-700 text-stone-300 text-xs font-medium border border-stone-700 transition-colors"
          >
            Reset Pilihan
          </button>
        </div>
      ...
      <div v-if="compareList.length === 0" class="py-12 text-center text-stone-400 bg-stone-950/40 rounded-xl border border-stone-800/80">
        ...
        >
          <div>
            ...
            <div class="grid grid-cols-2 gap-2 mt-3">
              <div class="p-2.5 rounded-lg bg-stone-900 border border-stone-700 text-center">
    ...
    >
      <div v-if="detailItem" class="space-y-5">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div class="p-3 rounded-lg bg-stone-50 border border-stone-200 text-center">
          ...
          <div class="p-3 rounded-lg bg-stone-900 border border-stone-800 text-center text-white">
